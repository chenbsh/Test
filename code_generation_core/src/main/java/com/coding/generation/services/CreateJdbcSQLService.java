...
public class CreateJdbcSQLService {
	...
	public Boolean constractSql(String databaseName, String tableName, String tableComment, Map<String, TableColumnBean> columnMap, String defaultMethod[]) throws SYHCException {
		...
		for (TableColumnBean column : columnMap.values()) {
			if (StringUtils.isNotBlank(column.getRemoveMothod())) {
				this.deleteSQL(databaseName, tableName, tableComment, columnMap.values(), sqlList);
			}
		}
	...
	private void deleteSQL(String databaseName, String tableName, String tableComment, Collection<TableColumnBean> columnList, List<String> sqlList) {
		Boolean isFirst = true;
		...
		for (TableColumnBean column : columnList) {
			if (column.getPrimaryKey()) {
				if (isFirst == false) {
					where.append(" and ");
					columnComment = columnComment + "和" + column.getColumnComment();
					columnName = columnName + "And" + column.getColumnName().substring(0, 1).toUpperCase() + column.getColumnName().substring(1, column.getColumnName().length());
				} else {
					isFirst = false;
